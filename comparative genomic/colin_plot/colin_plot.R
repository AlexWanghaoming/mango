...
# c("#D4AF37", "#40BCD8", "#39A9DB", "#FBCAEF", "#DC98D9", "#F865B0")
#-------------------------------------------------------------------------
# Add track Function
#-------------------------------------------------------------------------
addtrack <- function(track, galign_info_file, track_col,  min_ali_length=10000, ...) {
  ...
  # galign <- read.table("/Users/alexwang/0data/0mango/genome/mummer/yn56_yn56.bundle_telo.txt",header = T)
  ...
  for (i in 1:galign_length) {
    ...
    if (reorder_ali_len > min_ali_length & ali_ali_len > min_ali_length) {
      ...
      if (idx <= max_refseq_num) {
        rect(a$s1, idx-height+track*intvl+idx*gap, a$e1, idx-space+track*intvl+idx*gap, border = "gray", lwd = 0.4, col = track_col)
        if(a$bundle_size == 1){
        ...
        text((a$s1+a$e1)/2, idx-space+(track - 0.5)*intvl+idx*gap, a$ali, cex = 0.5)
         }
    }
  }
  ##  add telo
...
genome_info_file <- "/Users/alexwang/0data/0mango/genome/mummer/genome/yn56.genome.txt"
repeat_file <- "/Users/alexwang/0data/0mango/genome/repeat/yn56.repeat.bed"
...
pdf("yn56.pdf", width = 15, height = 8)
# prepare
...
# genome layout
cov <- read.table("/Users/alexwang/0data/0mango/genome/coverage/yn56_bin.txt")
cov[cov$chr==3,]
colnames(cov) <- c("chr","start","end","me","max","min")
cov$chr <- as.numeric(sub("yn56_","",cov$chr))
for (idx in seq(1, max_refseq_num)) {
  ...
  text(-200000, idx + -0.5 * intvl+gap*idx,  paste("yn56", refseq$chr[idx], sep = "_"), adj = 0, cex = 0.6)
  text(-200000, idx +  0.5 * intvl+gap*idx,  "qz", adj = 0, cex = 0.6)
  text(-200000, idx +  1.5 * intvl+gap*idx,  "gd10", adj = 0, cex = 0.6)
  text(-200000, idx +  2.5 * intvl+gap*idx,  "fj11", adj = 0, cex = 0.6)
  text(-200000, idx +  3.5 * intvl+gap*idx,  "hn47", adj = 0, cex = 0.6)
  text(-200000, idx +  4.5 * intvl+gap*idx,  "gz15", adj = 0, cex = 0.6)
  text(-200000, idx +  5.5 * intvl+gap*idx,  "yn55", adj = 0, cex = 0.6)
}
addtrack(1, "/Users/alexwang/0data/0mango/genome/mummer/yn56_qz.bundle_telo2.txt", cols[2])
addtrack(2, "/Users/alexwang/0data/0mango/genome/mummer/yn56_gd10.bundle_telo2.txt", cols[3])
addtrack(3, "/Users/alexwang/0data/0mango/genome/mummer/yn56_fj11.bundle_telo2.txt", cols[4])
addtrack(4, "/Users/alexwang/0data/0mango/genome/mummer/yn56_hn47.bundle_telo2.txt", cols[5])
addtrack(5, "/Users/alexwang/0data/0mango/genome/mummer/yn56_gz15.bundle_telo2.txt", cols[6])
addtrack(6, "/Users/alexwang/0data/0mango/genome/mummer/yn56_yn55.bundle_telo2.txt", cols[6])
